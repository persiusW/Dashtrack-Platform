  ...
  return (
    <AppLayout>
      <div className="space-y-8">
        ...
        {/* Organization */}
        {/* Organization (create if missing / otherwise read-only) */}
        <section className="rounded-xl border p-6 bg-white">
          <h2 className="font-semibold">Organization</h2>
          <p className="text-sm text-gray-600">
            {orgName ? "Your organization (read-only)" : "Create your organization to start activations"}
          </p>

          {orgName ? (
            <div className="mt-4 max-w-lg">
              <label className="text-sm text-gray-600">Organization Name</label>
              <input className="w-full border rounded px-3 py-2 bg-gray-50" value={orgName} disabled />
            </div>
          ) : (
            <OrgCreate />
          )}
